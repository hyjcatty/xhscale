...
// device view buttons
    ...
    $('#ScaleConfirmBtn').on('click',function() {
        var statcode = $(this).attr("StateCode");
...
function query_static_warning(){
    ...
    jQuery.get(request_head, map, function (data) {
        ...
        for( i=0;i<TableData.length;i++){
            ...
            txt = txt +"<td><button type='button' class='btn btn-default scale_start_btn' StateCode='"+TableData[i][0]+"' ><em class='glyphicon glyphicon-play ' aria-hidden='true' ></em></button></td><td><button type='button' class='btn btn-default scale_stop_btn' StateCode='"+TableData[i][0]+"' ><em class='glyphicon glyphicon-stop ' aria-hidden='true' ></em></button></td>";
        ...
        scale_start_btn_click = function(){
            var statcode = $(this).attr('StateCode');
            //console.log("StateCode_click="+statcode);
            if(statcode!==undefined ){
                show_startscalemodule(statcode);
            }
        };
        scale_stop_btn_click = function(){
            var statcode = $(this).attr('StateCode');
            //console.log("StateCode_click="+statcode);
            if(statcode!==undefined ){
                show_stopscalemodule(statcode);
            }
        };
        $(".scale_start_btn").on('click',scale_start_btn_click);
        $(".scale_stop_btn").on('click',scale_stop_btn_click);
...
function show_startscalemodule(statcode){
    $("#ScaleConfirmBtn").attr("StateCode",statcode);
    $("#ScaleConfirmBtn").attr("action","start");
    $("#ScaleConfirmModalContent").empty();
    $("#ScaleConfirmModalContent").append("<strong>请确认：</strong>是否要启动站点["+statcode+"]的智能秤！按确认继续。");

    modal_middle($('#ScaleConfirm'));
    $('#ScaleConfirm').modal('show') ;
}
function show_stopscalemodule(statcode){
    $("#ScaleConfirmBtn").attr("StateCode",statcode);
    $("#ScaleConfirmBtn").attr("action","stop");
    $("#ScaleConfirmModalContent").empty();
    $("#ScaleConfirmModalContent").append("<strong>请确认：</strong>是否要关闭站点["+statcode+"]的智能秤！按确认继续。");

    modal_middle($('#ScaleConfirm'));
    $('#ScaleConfirm').modal('show') ;
}

function startscale(statcode){
    var body={StatCode:statcode};
    var map = {
        action: "StartScale",
        body:body,
    ...
    var startscale_callback = function(result){
        var ret = result.status;
        if(ret == "true"){
            var auth = result.auth;
            if(auth == "true"){
                show_alarm_module(false,"已向下位机发送启动智能秤请求，请稍后刷新！",null);
            }else{
                show_alarm_module(true,"无效授权！"+result.msg,null);
            }

        }else{
            show_alarm_module(true,"启动智能秤请求失败，请重新登录！"+result.msg,null);
        }
    };
    JQ_get(request_head,map,startscale_callback);
}
function stopscale(statcode){
    var body={StatCode:statcode};
    var map = {
        action: "StopScale",
        body:body,
    ...
    var stopscale_callback = function(result){
        var ret = result.status;
        if(ret == "true"){
            var auth = result.auth;
            if(auth == "true"){
                show_alarm_module(false,"已向下位机发送关闭智能秤请求，请稍后刷新！",null);
            }else{
                show_alarm_module(true,"无效授权！"+result.msg,null);
            }

        }else{
            show_alarm_module(true,"关闭智能秤请求失败，请重新登录！"+result.msg,null);
        }
    };
    JQ_get(request_head,map,stopscale_callback);
}
//Key auth view
